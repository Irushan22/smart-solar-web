...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  const title = `${systemSize} Solar Installation in ${project.location}`;
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      images: [{ url: project.image, alt: `${systemSize} solar panel installation in ${project.location}` }],
      type: 'article',
    },
    twitter: {
    ...
    alternates: {
      canonical: `${siteConfig.site.url}/projects/${project.id}`,
    },
  };
}
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              { '@type': 'ListItem', position: 3, name: `${systemSize} Solar Installation in ${project.location}` },
            ],
          }),
        }}
      />
        ...
        <div className={styles.container}>
          ...
          <div className={styles.header}>
            ...
            <h1 className={styles.title}>
              {project.title || `${systemSize} Solar Installation in ${project.location}`}
            </h1>
          ...
          <div className={styles.gallery}>
            {project.images.slice(0, 4).map((img, index) => (
              <div key={img} className={styles.galleryImageWrapper}>
                <Image
                  src={img}
                  alt={`${systemSize} solar panel installation in ${project.location} - Image ${index + 1}`}
                  fill
